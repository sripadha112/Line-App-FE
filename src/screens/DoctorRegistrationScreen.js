...
export default function DoctorRegistrationScreen({ navigation, route }) {
  ...
  const [dropdownVisible, setDropdownVisible] = useState({
    ...
    timeDropdowns: {}   // Object to track time dropdowns by workspace index and time type
  });
  ...
  const toggleDropdown = (type, index = null, timeType = null, timeField = null) => {
    ...
    } else if (type === 'timeDropdown' && index !== null && timeType && timeField) {
      const key = `${index}_${timeType}_${timeField}`;
      setDropdownVisible(prev => ({
        ...prev,
        timeDropdowns: {
          ...prev.timeDropdowns,
          [key]: !prev.timeDropdowns[key]
        }
      }));
    } else {
      setDropdownVisible(prev => ({
  ...
  const selectOption = (type, value, index = null, timeType = null, timeField = null) => {
    if (type === 'workplaceType' && index !== null) {
      ...
      handleWorkspaceTimeChange(timeType, timeField, value, index);
      const key = `${index}_${timeType}_${timeField}`;
      setDropdownVisible(prev => ({
        ...prev,
        timeDropdowns: {
            ...
            <View style={styles.timeRow}>
              <View style={styles.timeGroup}>
                ...
                <View style={styles.timeInputRow}>
                  <TouchableOpacity
                    ...
                    onPress={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'hour')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.morningStartTime.hour).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_morningStartTime_hour`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'hour')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'hour')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {HOUR_OPTIONS.map((hour) => (
                            <TouchableOpacity
                              key={hour}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', hour, index, 'morningStartTime', 'hour')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(hour).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
                        </ScrollView>
                      </View>
                  ...
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'minute')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.morningStartTime.minute).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_morningStartTime_minute`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'minute')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'morningStartTime', 'minute')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {MINUTE_OPTIONS.map((minute) => (
                            <TouchableOpacity
                              key={minute}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', minute, index, 'morningStartTime', 'minute')}
                            >
              ...
              <View style={styles.timeGroup}>
                ...
                <View style={styles.timeInputRow}>
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'hour')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.morningEndTime.hour).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_morningEndTime_hour`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'hour')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'hour')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {HOUR_OPTIONS.map((hour) => (
                            <TouchableOpacity
                              key={hour}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', hour, index, 'morningEndTime', 'hour')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(hour).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
                  ...
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'minute')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.morningEndTime.minute).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_morningEndTime_minute`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'minute')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'morningEndTime', 'minute')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {MINUTE_OPTIONS.map((minute) => (
                            <TouchableOpacity
                              key={minute}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', minute, index, 'morningEndTime', 'minute')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(minute).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
            ...
            <View style={styles.timeRow}>
              <View style={styles.timeGroup}>
                ...
                <View style={styles.timeInputRow}>
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'hour')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.eveningStartTime.hour - 12).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_eveningStartTime_hour`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'hour')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'hour')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {HOUR_OPTIONS.map((hour) => (
                            <TouchableOpacity
                              key={hour}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', hour + 12, index, 'eveningStartTime', 'hour')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(hour).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
                  ...
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'minute')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.eveningStartTime.minute).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_eveningStartTime_minute`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'minute')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'eveningStartTime', 'minute')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {MINUTE_OPTIONS.map((minute) => (
                            <TouchableOpacity
                              key={minute}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', minute, index, 'eveningStartTime', 'minute')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(minute).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
              ...
              <View style={styles.timeGroup}>
                ...
                <View style={styles.timeInputRow}>
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'hour')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.eveningEndTime.hour - 12).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_eveningEndTime_hour`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'hour')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'hour')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {HOUR_OPTIONS.map((hour) => (
                            <TouchableOpacity
                              key={hour}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', hour + 12, index, 'eveningEndTime', 'hour')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(hour).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
                  ...
                  <TouchableOpacity
                    style={styles.timeDropdown}
                    onPress={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'minute')}
                  >
                    <Text style={styles.timeDropdownText}>
                      {String(workspace.eveningEndTime.minute).padStart(2, '0')}
                    </Text>
                  </TouchableOpacity>
                  
                  <Modal
                    visible={dropdownVisible.timeDropdowns[`${index}_eveningEndTime_minute`] || false}
                    transparent
                    animationType="fade"
                    onRequestClose={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'minute')}
                  >
                    <TouchableOpacity 
                      style={styles.modalOverlay}
                      onPress={() => toggleDropdown('timeDropdown', index, 'eveningEndTime', 'minute')}
                        ...
                        <ScrollView style={styles.dropdownList}>
                          {MINUTE_OPTIONS.map((minute) => (
                            <TouchableOpacity
                              key={minute}
                              style={styles.dropdownOption}
                              onPress={() => selectOption('timeDropdown', minute, index, 'eveningEndTime', 'minute')}
                            >
                              <Text style={styles.dropdownOptionText}>{String(minute).padStart(2, '0')}</Text>
                            </TouchableOpacity>
                          ))}
